...
fig4 <- function() {
  ...
  p3 <- ggplot(LungeKinematics::AllWhalesAvgs) +
    geom_pointrange(data = LungeKinematics::EnergyRatioLength,
  ...
  p4 <- ggplot(LungeKinematics::SpeciesEnRatioLength) +
    geom_pointrange(aes(log10(meanMass), log10(meanEnRatioDirectAdjust_mean),
                        color = Species,
                        ymin = log10(quartratio25),
                        ymax = log10(quartratio75))) +
    geom_point(aes(log10(meanMass), log10(meanEnRatioDirectAdjust_mean), fill = Species),
               color = "black",
               shape = 21,
               size = 10) +
