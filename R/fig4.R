...
fig4 <- function() {
  ...
  pal_ext <- c("Minke" = "#009E73",  "Humpback" = "#D55E00",  "Blue" = "#0072B2", "Fin" = "#9100B2", "Antarctic" = "Black", "Monterey" = "Gray", "Weddell" = "Red", "Otter" = "Green", "AfWildDog" = "Orange", "Lion" = "Blue")
  ...
  p1 <- ggplot(LungeKinematics::AllWhalesAvgs) +
    ...
    labs(x = "Log10 Body Length (m)",
         y = "Log10 Energy In/Out Per Lunge (kJ)") +
  ...
  p2 <- ggplot(LungeKinematics::AllWhalesAvgs) +
    ...
    scale_color_manual(values = pal, aesthetics = c("fill","colour")) +
    labs(x = "Log10 Body Length (m)",
         y = "Log10 Mass-Specific Energy In/Out Per Lunge (kJ/kg)") +
    theme_classic(base_size = 8) +
    theme(axis.text = element_text(size = 30),
          axis.title = element_text(size = 36),
          legend.position = "none",
  ...
  p3 <- ggplot(LungeKinematics::SpeciesEnRatioLength) +
    ...
    geom_point(data = LungeKinematics::LitEnRatios, aes(log10(Mass), log10(EnRatio), fill = Species), color = "black", shape = 23, size = 10) +
