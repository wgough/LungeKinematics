#' Create Supplementary Figure 7 for Gough et al. 2022 - Variance Testing Mouth Closing Speed
#' # "Minke" = "#009E73"
#' # "Humpback" = "#D55E00"
#' # "Blue" = "#0072B2"
#'
...
figS7 <- function() {
  ...
  sample.n = function(n){sample(VarianceMinke$SpdEndDecel, n, replace=FALSE)}
  ...
  for(i in 2:length(VarianceMinke$SpdEndDecel)){
  ...
  p1 <- ggplot(df.sample.var, aes(x=n, y=sample.variance))+
    ...
    ylim(0,1.5) +
    labs(x = "n",
         y = "Sample Variance - Speed of Mouth Closing") +
  ...
  sample.n = function(n){sample(VarianceHumpback$SpdEndDecel, n, replace=FALSE)}

  # create and empty dataframe to store results
  df.sample.var = data.frame()

  # for loop to fill in df.sample.var with variances using the var function
  for(i in 2:length(VarianceHumpback$SpdEndDecel)){
    tempDF <- data.frame(sample.variance=replicate(1000,var(sample.n(i))), n=i)
    if(i==2) df.sample.var <- tempDF else df.sample.var <-rbind(df.sample.var, tempDF)
  }
  ...
  sample.n = function(n){sample(VarianceBlue$SpdEndDecel, n, replace=FALSE)}

  # create and empty dataframe to store results
  df.sample.var = data.frame()

  # for loop to fill in df.sample.var with variances using the var function
  for(i in 2:length(VarianceBlue$SpdEndDecel)){
    tempDF <- data.frame(sample.variance=replicate(1000,var(sample.n(i))), n=i)
    if(i==2) df.sample.var <- tempDF else df.sample.var <-rbind(df.sample.var, tempDF)
  }
  ...
  ggsave("figs/Figure_Supp_7.pdf", height = 480, width = 960, units = "mm", dpi = 300)
}
