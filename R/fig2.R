...
fig2 <- function() {
  ...
  p3 <- ggplot(LungeKinematics::AllWhalesLungeTableTrunc, aes(log10(SpdBegDecel),log10(EnPerLungeDirectAdjust))) +
    geom_point(aes(fill = Species, size = TotLength), color = "black", shape = 21, alpha = 0.5) +
    geom_smooth(method = lm, aes(group = whaleName, colour = Species, fill = Species), se = FALSE) +
    scale_color_manual(values = pal, aesthetics = c("fill","colour")) +
    labs(x = bquote("Log"^10*" Speed At Mouth Opening (m s"^-1*")"),
  ...
  p4 <- ggplot(LungeKinematics::AllWhalesLungeTableTrunc, aes(log10(SpdBegDecel),log10(EnPerLungeDirectAdjustMS))) +
    geom_point(aes(fill = Species, size = TotLength), color = "black", shape = 21, alpha = 0.5) +
    geom_smooth(method = lm, aes(group = whaleName, colour = Species, fill = Species), se = FALSE) +
    scale_color_manual(values = pal, aesthetics = c("fill","colour")) +
    labs(x = bquote("Log"^10*" Speed At Mouth Opening (m s"^-1*")"),
  ...
  p5 <- ggplot(LungeKinematics::AllWhalesLungeTableTrunc, aes(log10(SpdBegDecel),log10(ModelWaterEngulfFullDirectAdjust))) +
    geom_point(aes(fill = Species, size = TotLength), color = "black", shape = 21, alpha = 0.5) +
    geom_smooth(method = lm, aes(group = whaleName, colour = Species, fill = Species), se = FALSE) +
    scale_color_manual(values = pal, aesthetics = c("fill","colour")) +
    labs(x = bquote("Log"^10*" Speed At Mouth Opening (m s"^-1*")"),
  ...
  p6 <- ggplot(LungeKinematics::AllWhalesLungeTableTrunc, aes(log10(SpdBegDecel),log10(EnRatioDirectAdjust))) +
    geom_point(aes(fill = Species, size = TotLength), color = "black", shape = 21, alpha = 0.5) +
    geom_smooth(method = lm, aes(group = whaleName, colour = Species, fill = Species), se = FALSE) +
    scale_color_manual(values = pal, aesthetics = c("fill","colour")) +
    labs(x = bquote("Log"^10*" Speed At Mouth Opening (m s"^-1*")"),
  ...
  p13 <- cowplot::plot_grid(p1, p3, p5, p2, p4, p6,
                           nrow = 2,
                           ncol = 3,
                           align = "v",
                           axis = "bl",
